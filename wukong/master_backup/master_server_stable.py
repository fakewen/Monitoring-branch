...
class Test_array:
  def __init__(self,name,n_id,pt,loc,num):
    self.id = str(n_id)+'_'+str(pt)
    self.sensor = name
    self.loc=loc
    #self.obj_array = wkpf.globals.mongoDBClient.wukong.readings.find({ 'node_id':n_id , 'port':pt }).sort('_id',-1).limit(num)
...
class Monitoring_Chart(tornado.web.RequestHandler):
  def get(self, nodeID, port):
      ...
      obj1 = Test('Light Sensor',int(nodeID),int(port),comm.getLocation(int(nodeID)))#location tree
