...
class Test:
  def __init__(self,name,n_id,pt,loc):
    ...
    self.sensor = name
    self.loc=loc
    cur_data = wkpf.globals.mongoDBClient.wukong.readings.find({ 'node_id':n_id , 'port':pt }).sort('timestamp',-1)
    self.value = cur_data.limit(1)[0]['value']
    self.ts=cur_data.limit(1)[0]['timestamp']
...
class Monitoring(tornado.web.RequestHandler):
  def get(self):
      ...
      for node in node_infos:
        print node.id #id[2]
        print node.location #location[4]
        for port_number in node.wuobjects.keys():
          wuobject = node.wuobjects[port_number]
          print 'port:', port_number #port[3]
          print wuobject.wuclassdef.name #sensor type[1]
          #tmp_obj=Test( str(wuobject.wuclassdef.name) , int(node.id), int(port_number) , str(node.location) )
          #self.ass.append(tmp_obj)
...
class GetValue(tornado.web.RequestHandler):
  def get(self):
      ...
      self.render('templates/value.html', applications=[obj2.value,obj2.ts])
