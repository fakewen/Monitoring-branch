...
class Test:
  def __init__(self,name,n_id,pt,loc):
    ...
    self.ts = wkpf.globals.mongoDBClient.wukong.readings.find({ 'node_id':n_id , 'port':pt }).sort('_id',-1).limit(1)[0]['timestamp']
...
class Test_array:
  def __init__(self,name,n_id,pt,loc):
    ...
    print "CountT"
    print self.count
    tmp=wkpf.globals.mongoDBClient.wukong.readings.find({ 'node_id':n_id , 'port':pt }).sort('_id',-1).limit(1)
    for i in range(self.count):
      self.value_array.append(tmp[i]['value'])
      self.time_array.append(tmp[i]['timestamp'])
...
class Test_line:
  def __init__(self,name,n_id,pt,loc):
    ...
    self.range_start_s="";
    self.range_end_s="";
    self.range_start=datetime.datetime.today()
    self.range_end=datetime.datetime.today()
    self.id = str(n_id)+'_'+str(pt)
    self.sensor = name
    self.loc=loc
    self.value_array=[]
    self.time_array=[]
    self.test_count1=wkpf.globals.mongoDBClient.wukong.readings.find({'node_id':n_id}).count()

    #self.hrago_count=?
    t1 = time.time()
#    self.count=wkpf.globals.mongoDBClient.wukong.readings.find({ 'node_id':n_id , 'port':pt }).limit(1).count()
    
    wkpf.globals.mongoDBClient.wukong.readings.ensure_index([("node_id",1),("port",1),("timestamp",1),("_id",-1)])
    tmp=list(wkpf.globals.mongoDBClient.wukong.readings.find( { 'node_id':n_id , 'port':pt ,'timestamp':{'$gte': datetime.datetime.today()+ datetime.timedelta(days=-1) }}, {"value":1, "timestamp":1, "_id":0}).sort("_id",-1))

    t2 = time.time()
    self.count=len(tmp)
    self.hrago_count=self.count
    print "Documents count=%d" %(self.count)
    #print "/The latest Documents=%d" %(tmp[0]["value"]) #the latest new
    hrago=datetime.datetime.today()+ datetime.timedelta(hours=-1)
    token=1
    for idx,i in enumerate(tmp): #range(self.count):
      self.value_array.append(i['value'])
      self.time_array.append(i['timestamp']) 
      if i['timestamp']<hrago and token:
        token=0;
        self.hrago_count=idx+1

    t3 = time.time()
    print "self.hrago_count=%d" %(self.hrago_count)
    print "It cost %f sec [Mongo query]" % (t2 - t1)
    print "It cost %f sec [pop array&timestamp to lists]" % (t3 - t2)
    print "Memory used %d Bytes" % (sys.getsizeof(tmp))
    '''
    for i in range(self.count):
      if type(tmp[i]['timestamp']) is LongType :
        try:
          #print "LONG:%f" %(tmp[i]['timestamp'])
          wkpf.globals.mongoDBClient.wukong.readings.update({'_id':tmp[i]["_id"]}, {"$set": {"timestamp":datetime.datetime.strptime( str(tmp[i]['timestamp']), "%Y%m%d%H%M%S")}}, upsert=False)
        except:
          print "[Long_TypeError!]%s" %(tmp[i]["_id"])
      elif type(tmp[i]['timestamp']) is UnicodeType :
        #print "STRING%s" %(tmp[i]['timestamp'])
        try:
          wkpf.globals.mongoDBClient.wukong.readings.update({'_id':tmp[i]["_id"]}, {"$set": {"timestamp":datetime.datetime.strptime(tmp[i]['timestamp'], "%Y%m%d%H%M%S")}}, upsert=False)
          #print "STRING>DATE[FOR SURE]%s:%s" %(type(tmp[i]['timestamp']),tmp[i]["_id"])
        except:
          print "[String_TypeError!]%s" %(tmp[i]["_id"])
          #2015-02-03 18:34:03
          wkpf.globals.mongoDBClient.wukong.readings.update({'_id':tmp[i]["_id"]}, {"$set": {"timestamp":datetime.datetime.strptime(tmp[i]['timestamp'], "%Y-%m-%d %H:%M:%S")}}, upsert=False)
      elif type(tmp[i]['timestamp']) is datetime.datetime :
        #print "datetime.datetime:%s" %(tmp[i]['timestamp'])
        pass
      else:
        print "else:%s;%s" %(type(tmp[i]['timestamp']),tmp[i]['id'])
      '''
class Test_line_month:
  def __init__(self,name,n_id,pt,loc):
    self.date_s=""
    self.time_s=""
    self.date_e=""
    ...
    self.value_array=[]
    self.time_array=[]
    self.test_count1=wkpf.globals.mongoDBClient.wukong.readingsbymin.find({'_id.node_id':n_id}).count()
    print "[HEAD][month]Total amount=%d" %(self.test_count1)
    t1 = time.time()
    wkpf.globals.mongoDBClient.wukong.readingsbymin.ensure_index([("_id.node_id",1),("_id.port",1),("_id.timestamp",1),("_id",-1)])
    tmp=list(wkpf.globals.mongoDBClient.wukong.readingsbymin.find( { '_id.node_id':n_id , '_id.port':pt ,'_id.timestamp':{'$gte': datetime.datetime.today()+ datetime.timedelta(days=-30) }}).sort("_id.timestamp",-1))
    
    print "MONTH = %s;value=%f;ts=%s" %(tmp[0],tmp[0]["value"],tmp[0]["_id"]["timestamp"])
    t2 = time.time()
    self.count=len(tmp)
    self.hrago_count=self.count
    print "[month]Documents count=%d" %(self.count)
    #print "/The latest Documents=%d" %(tmp[0]["value"]) #the latest new
    hrago=datetime.datetime.today()+ datetime.timedelta(weeks=-1)
    token=1
    for idx,i in enumerate(tmp): #range(self.count):
      self.value_array.append(i['value'])
      self.time_array.append(i["_id"]["timestamp"]) 
      if i["_id"]["timestamp"]<hrago and token:
        token=0;
        self.hrago_count=idx+1

    t3 = time.time()
    print "[month]It cost %f sec [Mongo query]" % (t2 - t1)
    print "[month]It cost %f sec [pop array&timestamp to lists]" % (t3 - t2)
    print "[month]Memory used %d Bytes" % (sys.getsizeof(tmp))

class Test_line_inrange:
  def __init__(self,name,n_id,pt,loc,range_start,range_end):
    self.id = str(n_id)+'_'+str(pt)
    self.sensor = name
    self.loc=loc
    self.value_array=[]
    self.time_array=[]
    self.date_s=range_start.split("T")[0]
    self.time_s=range_start.split("T")[1].split("Z")[0]
    self.date_e=range_end.split("T")[0]
    self.time_e=range_end.split("T")[1].split("Z")[0]
    self.range_start_s=range_start;
    self.range_end_s=range_end;
    self.range_start=dateutil.parser.parse(range_start)
    self.range_end=dateutil.parser.parse(range_end)
    print "[TTTTTTTT]%s ; %s " %(datetime.datetime.today(),self.range_start)
    self.test_count_org=wkpf.globals.mongoDBClient.wukong.readings.find({'node_id':n_id,'port':pt,'timestamp':{'$gte':self.range_start,'$lte':self.range_end}}).count()
    self.test_count_min=wkpf.globals.mongoDBClient.wukong.readingsbymin.find({'_id.node_id':n_id,'_id.port':pt,'_id.timestamp':{'$gte':self.range_start,'$lte':self.range_end}}).count()
    #if org>20000 , choose min
    if self.test_count_org>20000:
      print "[>20,000][>20,000][>20,000][>20,000][>20,000][>20,000][>20,000]"
      print "[HEAD][month]Total amount=%d" %(self.test_count_min)
      t1 = time.time()
      wkpf.globals.mongoDBClient.wukong.readingsbymin.ensure_index([("_id.node_id",1),("_id.port",1),("_id.timestamp",1),("_id",-1)])
      tmp=list(wkpf.globals.mongoDBClient.wukong.readingsbymin.find( { '_id.node_id':n_id , '_id.port':pt ,'_id.timestamp':{'$gte':self.range_start,'$lte':self.range_end}}).sort("_id.timestamp",-1))
      ...
      print "[<20,000][>20,000][>20,000][>20,000][>20,000][>20,000][>20,000]"
      print "[HEAD][month]Total amount=%d" %(self.test_count_org)
      t1 = time.time()
      wkpf.globals.mongoDBClient.wukong.readings.ensure_index([("node_id",1),("port",1),("timestamp",1),("_id",-1)])
      tmp=list(wkpf.globals.mongoDBClient.wukong.readings.find( { 'node_id':n_id , 'port':pt ,'timestamp':{'$gte':self.range_start,'$lte':self.range_end}}).sort("_id",-1))
...
class Monitoring_Line_All_Inrange(tornado.web.RequestHandler):
  def get(self):
      print "[Check Date] %s ; %s TO %s ; %s" %((self.get_argument("arg1")),(self.get_argument("arg2")),(self.get_argument("arg3")),(self.get_argument("arg4")))
      print datetime.datetime.today()
      range_start=(self.get_argument("arg1"))+"T"+(self.get_argument("arg2")+"Z")
      range_end=(self.get_argument("arg3"))+"T"+(self.get_argument("arg4")+"Z")
      print dateutil.parser.parse(range_start)
      print dateutil.parser.parse(range_end)
      comm = getComm()
      node_infos = comm.getAllNodeInfos(False)
      obj=[]    
      tStart = time.time()
      for i in range(MONITORING_COUNT):
        obj.append( Test_line('Light Sensor',MONITORING_NODE[i],MONITORING_PORT[i],comm.getLocation(MONITORING_NODE[i])) );
      for i in range(MONITORING_COUNT):
        obj.append( Test_line_month('Light Sensor',MONITORING_NODE[i],MONITORING_PORT[i],comm.getLocation(MONITORING_NODE[i])) );
      for i in range(MONITORING_COUNT):
        obj.append( Test_line_inrange('Light Sensor',MONITORING_NODE[i],MONITORING_PORT[i],comm.getLocation(MONITORING_NODE[i]),range_start,range_end) );
      
      tEnd = time.time()
      print "It cost %f sec" % (tEnd - tStart)
      print tEnd - tStart

      #print obj[0].time_array
      self.render('templates/line_all_inrange.html', applications=obj)
...
class Monitoring_Line_All(tornado.web.RequestHandler):
  def get(self):
      ...
      #print obj[0].time_array
      self.render('templates/line_all.html', applications=obj)
...
class Monitoring_Line_Check(tornado.web.RequestHandler):
  def get(self):
      comm = getComm()
      node_infos = comm.getAllNodeInfos(False)
      obj=[]    
      ...
      print tEnd - tStart
      #print obj[0].time_array
      self.render('templates/line_check.html', applications=obj)

class asd(tornado.web.RequestHandler):
  def get(self):
      obj=[]    
      self.render('templates/asd.html', applications=obj)
...
class Monitoring_Line(tornado.web.RequestHandler):
  def get(self, nodeID, port):
      ...
      obj=[]    
      tStart = time.time()
      obj.append( Test_line('Light Sensor',int(nodeID),int(port),comm.getLocation(int(nodeID))) );
      tEnd = time.time()
      print "It cost %f sec" % (tEnd - tStart)
      print tEnd - tStart

      #print obj[0].time_array
      self.render('templates/index4.html', applications=obj)
...
class Monitoring_Planar(tornado.web.RequestHandler):
  def get(self):
      ...
      for i in range(MONITORING_COUNT):
        obj.append( Test('Light Sensor',MONITORING_NODE[i],MONITORING_PORT[i],"7F") )
...
class GetValue(tornado.web.RequestHandler):
  def get(self):
      ...
      #http://localhost:5004/getvalue?arg1=IR%20Sensor&arg2=3&arg3=2&arg4=BL-7F%20entrance
...
wukong = tornado.web.Application([
  ...
  ,(r"/monitoring_line/([0-9]*)/([0-9]*)",Monitoring_Line)
  ,(r"/monitoring_line",Monitoring_Line_All)
  ,(r"/monitoring_line_inrange",Monitoring_Line_All_Inrange)
  ,(r"/monitoring_check",Monitoring_Line_Check)
  ,(r"/asd",asd)
  ,(r"/monitoring_planar",Monitoring_Planar)
  ,(r"/getvalue",GetValue)
  ,(r"/getvalue_array",GetValue_array)
  ,(r'/data\.tsv', MyFileHandler, {'path': './data/data.tsv'})
], IP, **settings)
