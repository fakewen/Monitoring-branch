...
class Test_line:
  def __init__(self,name,n_id,pt,loc):
    ...
    wkpf.globals.mongoDBClient.wukong.readings.ensure_index([("node_id",1),("port",1),("_id",-1)])
    tmp=list(wkpf.globals.mongoDBClient.wukong.readings.find( { 'node_id':n_id , 'port':pt }).sort("_id",-1))
    print "/The latest Documents=%d" %(tmp[0]["value"]) #the latest new
    for i in range(self.count):
      #tmp[i]['value']
      self.value_array.append(tmp[i]['value'])
      #print "/value:%d" %(tmp[i]['value'])
      #tmp[i]['timestamp']
      self.time_array.append(tmp[i]['timestamp']) 
      #print "/timestamp:%s" %(tmp[i]['timestamp'])

    '''
    for i in range(self.count):
      if type(tmp[i]['timestamp']) is LongType :
        print "LONG:%f" %(tmp[i]['timestamp'])
        #wkpf.globals.mongoDBClient.wukong.readings.update({'_id':tmp[i]["_id"]}, {"$set": post}, upsert=False)
        ...
        try:
          ...
          print "STRING>DATE[FOR SURE]%s" %(type(tmp[i]['timestamp']))
        except:
          print "[TypeError!]"
        ...
        print "else:%s;%s" %(type(tmp[i]['timestamp']),tmp[i]['timestamp'])
    '''
...
wukong = tornado.web.Application([
  ...
  ,(r"/monitoring_line",Monitoring_Line)
